...
// 1. Read the existing dict.ts
const dictPath = 'miniprogram/utils/dict.ts';
console.log('Reading dict.ts...');
const dictContent = fs.readFileSync(dictPath, 'utf8');

// Parse existing keys from BUILTIN_DICT to avoid duplicates
...
console.log('Parsed existing dictionary keys count:', existingKeys.size);

// 2. Load the generated scenarios
console.log('Loading generated scenarios from batches...');
...
// 3. Extract all Thai dialogue texts
const thaiTexts = [];
...
console.log('Total Thai texts collected:', thaiTexts.length);

// 4. Segment the texts using Intl.Segmenter
...
thaiTexts.forEach(text => {
  ...
  for (const { segment } of segments) {
    ...
    // Keep only segments that contain Thai characters
    if (word && /[\u0e00-\u0e7f]/.test(word)) {
...
console.log('Harvested unique missing Thai words count:', uniqueNewWords.size);

// Load the predefined vocabulary mapping
const vocabDb = require('./vocab_db.json');

// Helper to transliterate Thai letters to romanized phonetic representation as a fallback
...
// Map subsegments or fallback translations
const subsegments = {
...
for (const word of uniqueNewWords) {
  ...
  if (vocabDb[word]) {
    ...
    // Try to find if any of our keys contains this word or is contained in it
    let matched = false;
    ...
    if (!matched) {
      phonetic = transliterateThai(word);
      meaning = '词汇'; // Default short meaning
    }
  }

  // Ensure meaning is brief (1-4 characters) as requested
  if (meaning.length > 4) {
...
console.log('Formatted', addedCount, 'new dictionary entries to append.');

// 6. Find the insertion index in dict.ts (right before the last }; of BUILTIN_DICT)
// Let's locate:
// };
// 
// /**
//  * 查询单词
const markerMatch = dictContent.match(/\};\r?\n\r?\n\/\*\*\r?\n\s*\* 查询单词/);
if (!markerMatch) {
  throw new Error('Could not find insertion point in dict.ts');
}

const markerIndex = markerMatch.index;
const updatedDictContent = dictContent.substring(0, markerIndex + 2) + ',' + entriesToAppend.trimEnd() + '\n' + dictContent.substring(markerIndex + 2);

fs.writeFileSync(dictPath, updatedDictContent, 'utf8');
console.log('Successfully updated dict.ts with new harvested words.');
